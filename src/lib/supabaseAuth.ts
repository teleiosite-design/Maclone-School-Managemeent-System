...
const validRoles = Object.keys(rolePaths) as UserRole[];
...
export function resolveUserRole(user: SupabaseUser): UserRole | null {
...
function createSession(payload: TokenResponse) {
  const role = resolveUserRole(payload.user);

  if (!role) {
    throw new Error("Your account does not have a portal role yet. Ask an administrator to set role to admin, teacher, student, or parent in Supabase user metadata.");
  }
...
export function getStoredSession(): AuthSession | null {
  ...
  try {
    ...
    if (!session.access_token || !session.refresh_token || !session.user?.id || !isUserRole(session.role)) return null;
    return session;
  } catch {
...
export async function refreshSession(session: AuthSession) {
  ...
  const payload = await parseAuthResponse(response) as TokenResponse;
  const refreshed = createSession(payload);
